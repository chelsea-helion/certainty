...
Our data is a bit complicated --- we have nine videos, which all differ in terms of their content. So we will have to fit separate models for all nine before we can start to make more general inferences about the nature of the association between role and moral certainty.

```{r}
video_split <- split(df_behav, df_behav$Video)
linear_results_list <- list()
model_formula <- "Rating ~ Condition + (1|PID)"

# linear model fits

for (i in seq_along(video_split)){
   model <- lmer(model_formula, data = video_split[[i]])
   linear_results_list[[i]] <- summary(model)
}

# print model fit results
for (i in seq_along(video_split)) {
  print(paste("Results for video", i))
  print(linear_results_list[[i]])
}
